...
if board == "TX1/Nano":
	...
	cpu_scaling_available_frequencies = [115200,192000,268800,345600,422400,499200,
											576000,652800,729600,806400,883200,960000,
											1036800,1113600,1190400,1267200,1344000,
											1420800,1497600,1574400,1651200,1728000,
											1804800,1881600,1907200]

	gpu_available_frequencies = [114750000,204000000,306000000,408000000,510000000,
											599250000,701250000,752250000,803250000,
											854250000,905250000,956250000,1007250000,
											1058250000,1109250000]
...
cpu_govs = ["interactive","conservative","ondemand","userspace","powersave","performance","schedutil"]
gpu_govs = ["wmark_simple","nvhost_podgov","userspace","performance","simple_ondemand"]
...
def set_gov(cpu_gov,gpu_gov):
	...
	print("WARNING!!! Changing governor is not fully optimized. Code cleaning is needed")

	subprocess.Popen("sudo nvpmodel -m 8",stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,shell=True)
	time.sleep(1)
	subprocess.Popen("sudo nvpmodel -m 8",stdin=subprocess.PIPE,shell=True)
	time.sleep(0.5)

	if cpu_gov not in cpu_govs or gpu_gov not in gpu_govs:
		raise Exception("Not a valid governor\nCPU:"+str(cpu_govs)+"\nGPU:"+str(gpu_govs))
...
if __name__ == '__main__':
	...
	if board == "TX1/Nano":
		...
		parser.add_argument('--cpus', 		action='store', type=int, default= 6, help="input can be 1 to 6" 	)
		parser.add_argument('--cpu_max_fq',	action='store', type=int, default=-1, help="input can be 0 to 24"	)
		parser.add_argument('--gpu_max_fq',	action='store', type=int, default=-1, help="input can be 0 to 14"	)

	parser.add_argument('--cpu_gov',	action='store', type=str, default="schedutil", help="input can be a cpu governor. \
					 					Possible values are: interactive, conservative, ondemand, userspace, powersave, performance, schedutil")
	parser.add_argument('--gpu_gov',	action='store', type=str, default="nvhost_podgov", help="input can be a gpu governor. \
					 					Possible values are: wmark_simple, nvhost_podgov, userspace, performance, simple_ondemand")
	parser.add_argument('--ONLY_GOV',	action='store', type=bool, default=False, help="Set only the governor")
	parser.add_argument('--ONLY_FREQ',	action='store', type=bool, default=False, help="Set only the frequency")
